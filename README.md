# template-backend-groovy

Backend template for [DynamiaTools](https://dynamia.tools) using Spring Boot and Groovy. This repository is
consumed by the [DynamiaTools CLI](https://github.com/dynamiatools/framework/tree/main/platform/packages/cli)
(`dynamia new`) to scaffold new Groovy backends — it is not meant to be used standalone, though it builds and its
...
- Java 25 (LTS)
- Groovy 5.1.3 (compiled via `gmavenplus-plugin`)
...
- The main source package must stay `com.example.demo` (under `src/main/groovy` and `src/test/groovy`) — the CLI
